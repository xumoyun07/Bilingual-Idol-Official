...
export default function News() {
  ...
  return <PublicLayout><div className="simple-route-page news-page"><header className="simple-route-header"><p className="simple-eyebrow">News</p><h1>Centre updates, notices and events.</h1><p>Read information published directly by the centre. Select a card for the full update.</p></header><section className="simple-route-section news-feed" aria-label="Centre news">
    ...
    {!feed.isLoading && !feed.error && feed.data?.rows.length ? <><div className="news-grid">{feed.data.rows.map(item => { const Icon = icons[item.category]; return <article className="news-card" key={item.id}><button type="button" className="news-card-trigger" onClick={() => setSelected(item)} aria-haspopup="dialog" aria-label={`Read ${item.title}`}>
  ...
  </section></div><Dialog open={selected !== null} onOpenChange={open => { if (!open) setSelected(null); }}><DialogContent className="news-dialog max-h-[calc(100dvh-2rem)] max-w-[calc(100%-1rem)] overflow-y-auto p-0 sm:max-w-2xl"><DialogHeader className="news-dialog-header">{selected ? <NewsDialogHeader post={selected} /> : null}</DialogHeader>{selected ? <div className="news-dialog-body">{selected.imageUrl ? <img src={selected.imageUrl} alt={selected.imageAltText || ""} /> : null}<div className="news-dialog-copy">{selected.body.split(/\n{2,}/).map((paragraph, index) => <p key={index}>{paragraph}</p>)}</div></div> : null}</DialogContent></Dialog></PublicLayout>;
}

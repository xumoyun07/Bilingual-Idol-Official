...
export function DynamicUserProfileFields({ fields, sections, values, onChange }: { fields: DynamicField[]; sections: DynamicSection[]; values: Record<string, string>; onChange: (key: string, value: string) => void }) {
  ...
  if (!activeFields.length) return <section className="mt-6 rounded-xl border border-dashed border-[#d8cfbf] bg-[#faf6ef] p-5"><div className="flex gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#e7f0eb] text-[#397563]"><ListChecks size={18} /></span><div><p className="font-bold text-[#10253e]">No more fields are configured.</p><p className="mt-1 text-sm leading-5 text-[#53657a]">Use Configure create form to create or edit fields and organise them into groups.</p></div></div></section>;

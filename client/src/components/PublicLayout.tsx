      ...
      <header className="sticky top-0 z-40 border-b border-[#e8dcc9]/80 bg-[#fbf8f2]/90 backdrop-blur-xl">
        <div className="site-container flex h-[76px] items-center justify-between gap-5">
          ...
          <div className="hidden items-center gap-3 lg:flex">
            ...
            <Link href="/admin/login" className="inline-flex items-center rounded-full border border-[#cbbba4] px-4 py-2.5 text-sm font-extrabold text-[#10253e] transition-colors hover:border-[#10253e] hover:bg-[#f2eade]" aria-label="Вход в Founder console">Вход</Link>
        ...
        {open && <nav className="border-t border-[#e8dcc9] bg-[#fbf8f2] px-4 pb-5 pt-3 lg:hidden" aria-label="Mobile navigation"><div className="site-container grid gap-1">{navigation.map(item => <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className={`rounded-xl px-4 py-3 text-sm font-extrabold ${location === item.href ? "bg-[#10253e] text-white" : "text-[#29415b] hover:bg-[#f2eade]"}`}>{item.label}</Link>)}<Link href="/admin/login" onClick={() => setOpen(false)} className="mt-2 inline-flex items-center justify-center rounded-xl border border-[#cbbba4] px-4 py-3 text-sm font-extrabold text-[#10253e] hover:bg-[#f2eade]">Вход</Link><Link href="/enroll" onClick={() => setOpen(false)} className="mt-2 inline-flex items-center justify-center gap-2 rounded-xl bg-[#f07e5d] px-4 py-3 text-sm font-extrabold text-[#10253e]">Enroll Now <ArrowUpRight size={16} /></Link></div></nav>}
      </header>

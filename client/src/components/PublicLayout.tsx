  ...
  return <div className="simple-public-shell">
    ...
    <header className="simple-public-header simple-public-header--refined">
      <div className="simple-public-bar">
        ...
        <div className="simple-public-actions"><a className="simple-contact-link" href="tel:+60367310449"><Phone size={16} aria-hidden="true" /> Call</a><Link href="/login" className="simple-button simple-button-quiet">Sign in</Link><Link href="/enroll" className="simple-button simple-button--green">Make an enquiry</Link></div>
      ...
      {open && <nav className="simple-mobile-nav" aria-label="Mobile navigation">{primaryNavigation.map(item => <Link key={item.href} href={item.href} onClick={close} aria-current={location === item.href ? "page" : undefined}>{item.label}</Link>)}<Link href="/login" onClick={close}>Sign in</Link><Link href="/enroll" className="simple-button simple-button--green" onClick={close}>Make an enquiry</Link></nav>}
    </header>

...
const postInput = z.object({ slug: z.string().trim().toLowerCase().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).max(180), title: z.string().trim().min(3).max(220), excerpt: z.string().trim().min(10).max(1_000), body: z.string().trim().min(20).max(20_000), category, isPublished: z.boolean(), publishedAt: z.coerce.date().nullable().optional(), image: imageInput.optional(), clearImage: z.boolean().default(false) });
...
export const newsRouter = router({
  ...
  create: founderProcedure.input(postInput).mutation(async ({ ctx, input }) => {
    try {
      ...
      const record = await db.createAnnouncement({ slug: input.slug, title: input.title, excerpt: input.excerpt, body: input.body, category: input.category, isPublished: input.isPublished, publishedAt: input.isPublished ? input.publishedAt ?? new Date() : null, ...media });
  ...
  update: founderProcedure.input(postInput.extend({ id: z.number().int().positive() })).mutation(async ({ ctx, input }) => {
    try {
      ...
      const record = await db.updateAnnouncement(input.id, { slug: input.slug, title: input.title, excerpt: input.excerpt, body: input.body, category: input.category, isPublished: input.isPublished, publishedAt: input.isPublished ? input.publishedAt ?? existing.publishedAt ?? new Date() : null, ...media });
